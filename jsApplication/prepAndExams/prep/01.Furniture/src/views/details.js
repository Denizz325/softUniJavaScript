...
const detailTemplate = (itemPromise) => html` 
...
const detailCard = (item, isOwner, onDelete) => html`
<div class="col-md-4">
...
export function detailsPage(ctx) {
  ...
  async function onDelete() {
    ...
    if (choice) {
        ctx.render(deleteItem(ctx.params.id))
        ctx.page.redirect('/')
    }
    }
}
...
async function loadItem(id, onDelete) {
  ...
  return detailCard(item, isOwner, onDelete);
}
